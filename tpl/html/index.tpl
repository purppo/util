...
<html lang="jp">
  <head>
    ...
    <title>Source Code Highlight</title>
		...
		<div class="row">
			...
			<div>
				<form role="form" method="post">
					...
					<div class="col-sm-6">
						<select name="language" class="select_search form-control input-sm">
							{foreach from=$language_list key=k item=v}
								<option value="{$v}" {if $params.language == $v} selected {/if}>{$v}</option>
							{/foreach}}
						</select>
					</div>
					
					<div class="col-sm-6">
						<button class="btn btn-sm btn-warning btn-block" type="submit">Highlight</button>
					</div>
				</form>
			</div>
